<div id="userbar">
	...
	<?php else : ?>
		...
		<p id="login-text"><?php _e( 'Debes estar logueado para iniciar sesion.', 'buddypress' ) ?></p>
	
		<form name="loginform" id="loginform" action="<?php echo site_url('wp-login.php') ?>" method="post">
			<p>
				<label><?php _e( 'Nombre de usuario', 'buddypress' ) ?><br />
				<input type="text" name="log" id="user_login" class="input" value="<?php echo attribute_escape(stripslashes($user_login)); ?>" /></label>
			</p>
			<p>
				<label><?php _e( 'Contraseña', 'buddypress' ) ?><br />
				<input type="password" name="pwd" id="user_pass" class="input" value="" /></label>
			</p>
			<p class="forgetmenot"><label><input name="rememberme" type="checkbox" id="rememberme" value="forever" /> <?php _e( 'Recordarla', 'buddypress' ) ?></label></p>
			<p class="submit">
				<input type="submit" name="wp-submit" id="wp-submit" value="<?php _e('Entrar'); ?>" tabindex="100" />
